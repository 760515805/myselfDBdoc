...
//sHint=List out all related items by searching through item links
//sCategory=MainMenu.Search
//sLocaleID=p.ListAllRelated
//sAppVerMin=6.0.6
//sShortcutKey=
	...
	if(xNyf.isOpen()){
		...
		if(plugin.getCurNavigationTab()=='Outline'){
			...
			if(sCurItem){
				...
				var _list_related=function(sItem){
					var v=plugin.listRelated(-1, sItem);
					if(v){
						var v2=[];
						for(var i in v){
							var s=v[i];
